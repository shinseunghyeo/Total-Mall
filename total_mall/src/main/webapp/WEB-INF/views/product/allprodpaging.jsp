...
<body>
    ...
    <c:if test="${pageNav.pageNum > pageNav.pages_pageBlock}">
        <a class="page-link" href="../product/allplist.do?pageNum=1&pageBlock=1">&lt;&lt;</a>
        &nbsp;
        <a class="page-link" href="../product/allplist.do?pageNum=${(pageNav.pageBlock-2)*pageNav.pages_pageBlock+1}&pageBlock=${pageNav.pageBlock-1}">이전</a>
    </c:if>
    ...
    <c:forEach var="p" begin="${(pageNav.pageBlock - 1)*pageNav.pages_pageBlock +1}" end="${pageNav.pageBlock*pageNav.pages_pageBlock}">
        <c:if test="${p le pageNav.total_pageNum}">
            <a class="page-link" href="../product/allplist.do?pageNum=${p}&pageBlock=${pageNav.pageBlock}">
                <!-- 현재 페이지를 빨간색으로 출력 -->
    ...
    <c:if test="${pageNav.total_pageNum > (pageNav.pageBlock*pageNav.pages_pageBlock)}">
        <a class="page-link" href="../product/allplist.do?pageNum=${(pageNav.pageBlock*pageNav.pages_pageBlock)+1}&pageBlock=${pageNav.pageBlock + 1}">다음</a>
        &nbsp;
        <a class="page-link" href="../product/allplist.do?pageNum=${pageNav.total_pageNum}&pageBlock=${pageNav.last_pageBlock}">&gt;&gt;</a>
    </c:if>
