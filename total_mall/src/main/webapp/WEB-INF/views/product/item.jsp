			...
			<div id="content">
                ...
                var buttons = document.getElementsByClassName("rating-button");
                    function changeContent(option, clickedButton) {
                        ...
                        switch(option) {
                            case 'recent':
                                contentDiv.innerHTML = '<c:choose><c:when test="${empty hEvaluationList}"><tr><td colspan="6">등록된 리뷰가 없습니다</td></tr></c:when><c:otherwise><c:forEach items="${hEvaluationList}" var="review" varStatus="vs">${review.member_name}<br><c:forEach begin="1" end="5" varStatus="star"><c:choose><c:when test="${star.index <= review.star}"><span style="color: orange">★</span></c:when><c:otherwise><span style="color: orange">☆</span></c:otherwise></c:choose></c:forEach>(${review.star})<br><fmt:formatDate value="${review.post_date}"
                                ...
                                break;
                            case 'old':
                                contentDiv.innerHTML = '<c:choose><c:when test="${empty lEvaluationList}"><tr><td colspan="6">등록된 리뷰가 없습니다</td></tr></c:when><c:otherwise><c:forEach items="${lEvaluationList}" var="review" varStatus="vs">${review.member_name}<br><c:forEach begin="1" end="5" varStatus="star"><c:choose><c:when test="${star.index <= review.star}"><span style="color: orange">★</span></c:when><c:otherwise><span style="color: orange">☆</span></c:otherwise></c:choose></c:forEach>(${review.star})<br><fmt:formatDate value="${review.post_date}"
                                    pattern="yyyy-MM-dd HH:mm:ss" /><br><c:if test="${not empty review.originfile_name}"><img width="50" height="50" src="../resources/uploads/${review.savefile_name}"></c:if>${review.content}<hr></c:forEach></c:otherwise></c:choose>';
                                break;
                            case 'high_rating':
                                contentDiv.innerHTML = '<c:choose><c:when test="${empty hStarList}"><tr><td colspan="6">등록된 리뷰가 없습니다</td></tr></c:when><c:otherwise><c:forEach items="${hStarList}" var="review" varStatus="vs">${review.member_name}<br><c:forEach begin="1" end="5" varStatus="star"><c:choose><c:when test="${star.index <= review.star}"><span style="color: orange">★</span></c:when><c:otherwise><span style="color: orange">☆</span></c:otherwise></c:choose></c:forEach>(${review.star})<br><fmt:formatDate value="${review.post_date}"
                                    pattern="yyyy-MM-dd HH:mm:ss" /><br><c:if test="${not empty review.originfile_name}"><img width="50" height="50" src="../resources/uploads/${review.savefile_name}"></c:if>${review.content}<hr></c:forEach></c:otherwise></c:choose>';
                                break;
                            case 'low_rating':
                                contentDiv.innerHTML = '<c:choose><c:when test="${empty lStarList}"><tr><td colspan="6">등록된 리뷰가 없습니다</td></tr></c:when><c:otherwise><c:forEach items="${lStarList}" var="review" varStatus="vs">${review.member_name}<br><c:forEach begin="1" end="5" varStatus="star"><c:choose><c:when test="${star.index <= review.star}"><span style="color: orange">★</span></c:when><c:otherwise><span style="color: orange">☆</span></c:otherwise></c:choose></c:forEach>(${review.star})<br><fmt:formatDate value="${review.post_date}"
                                    pattern="yyyy-MM-dd HH:mm:ss" /><br><c:if test="${not empty review.originfile_name}"><img width="50" height="50" src="../resources/uploads/${review.savefile_name}"></c:if>${review.content}<hr></c:forEach></c:otherwise></c:choose>';
                                break;
                            default:
                                break;
